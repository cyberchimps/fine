...
// frontend display
function fine_testimonial_render_display()
{
	global $post;
        // Set directory uri
	...
	if(is_page())
	{
		$fine_testimonial_title = get_post_meta( $post->ID, 'ir_testimonial_title', true );
	...
	{
		$fine_testimonial_title = cyberchimps_get_option('ir_testimonial_title');
	...
	if(  !empty($fine_testimonial_title)  ){
	...
	<div id="fine_testimonial_top">
		<?php if( !empty($fine_testimonial_title) ) { ?>


				<?php if(!empty($fine_testimonial_title)) { ?>
				<h2 class="fine_main_title">
					<?php echo $fine_testimonial_title; ?>
				</h2>
				<?php } ?>
      ...
      <section class="fine_slider_text_img">
        <div id="slider2" class="flexslider">
          <ul class="slides">
			<?php	foreach( $testimonials as $testimonial) {
				...
				<li class="col-md-12">
                                    <span class="fine_testimonial_text"><?php if(!empty($testimonial['text'])){echo $testimonial['text'];} ?></span>
					<?php if(!empty($testimonial['text']))
							{ ?>
							<hr class="after_testimonial_text"> </hr>
					<?php } ?>
					<div class="fine_testimonial_author">
                                                <?php  if(!empty($testimonial['client'])){echo $testimonial['client'];} ?>
					</div>
					<div class="fine_testimonial_abt_author">
